...
- **Provider execution blocked at startup:** if saved provider qualification fails validation or the authority service cannot be reached during provider setup, the desktop opens without execution authority and displays a persistent warning. Local records remain available; uncertain work stays fenced. Restore service availability or renew qualification through supported setup, then restart Keeper. The desktop does not renew authorization, enable paid execution, or select a fallback automatically. Database/recovery identity errors still fail closed rather than entering this mode.
