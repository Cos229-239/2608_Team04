...
- Exact Founder disposition of an uncertain external execution requires an authenticated Provider Host recovery barrier that rejects live workers, advances a durable monotonic generation, and proves the exact Authority attempt and launch absent from the active-or-uncertain journal. The Founder approval binds its initial signed proof and the original execution charter separately from the current approval charter. KeeperAuthority validates the signed Executive database/recovery identity, obtains and stores a same-identity final barrier proof at an equal or newer generation, durably terminalizes the attempt, and signs the exact disposition before local claims are released. It must preserve that an effect may have occurred, accept no provider result, authorize no retry, consume the reserved usage upper bound, and bind every workspace and write claim in an immutable disposition record before releasing local concurrency.
- Recovery visibility is global: changing the selected project must never hide an unresolved external execution or reduce the displayed uncertainty count.
